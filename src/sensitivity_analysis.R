...
# --- Exclude the biologic-treated C cases -----------------------------------
# read.delim()/data.frame() run column headers through make.names(), which
# silently turns "-" into "." (e.g. "TCGA-AA-3844" -> "TCGA.AA.3844"), and
# build_coldata() keeps that raw formatting as its rownames. Normalize before
# matching, same fix as build_coldata() applies internally for its own
# sample-to-condition matching.
normalize_id <- function(x) gsub("[.-]", "-", x)

coldata_ids_norm <- normalize_id(rownames(coldata_full))
message("First few normalized coldata IDs: ", paste(head(coldata_ids_norm), collapse = ", "))
message("biologic_treated_C IDs: ", paste(biologic_treated_C, collapse = ", "))

keep <- !(coldata_ids_norm %in% biologic_treated_C)
n_excluded <- sum(!keep)

if (n_excluded != length(biologic_treated_C)) {
  stop(sprintf(
    "sensitivity_analysis: expected to exclude %d cases but matched %d - compare the ID lists printed above before proceeding.",
    length(biologic_treated_C), n_excluded
  ))
}
...
message(sprintf(
  ...
  sum(coldata_sens$condition == "S"), n_excluded
))

# --- DESeq2, same design/contrast/shrinkage convention as the original ------
